...
for bestMatchIndex=1:bestMatchSize
    ...
    if size(heteroIndex,2)>=5 && ~strcmp(combiTemp{1},combiTemp{2})
        ...
        save_name = [outputFolder,'/',artificialDoubletsCombiUnique{bestMatchUniqueTemp},'.mat'];
        niche_geness = gene_name(geneIndex);
        save(save_name , 'niche_geness')

        ofile = fopen([outputFolder,'/DEG_',artificialDoubletsCombiUnique{bestMatchUniqueTemp},'up_pvalue',num2str(pCutoff),'lr',num2str(lrCutoff),'_list.txt'],'w');
